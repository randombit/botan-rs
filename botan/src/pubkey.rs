...
impl Privkey {
    ...
    /// Load a kyber 512 private key
    ///
    /// # Examples
    ///
    /// ```
    /// let v = vec![0x42; 800];
    /// let key = botan::Privkey::load_kyber512(&v).unwrap();
    /// ```
    #[cfg(feature = "botan3")]
    pub fn load_kyber512(key: &[u8]) -> Result<Privkey> {
        let obj = botan_init!(botan_privkey_load_kyber512, key.as_ptr())?;
        Ok(Privkey { obj })
    }

    /// Load a kyber 768 private key
    ///
    /// # Examples
    ///
    /// ```
    /// let v = vec![0x42; 1184];
    /// let key = botan::Privkey::load_kyber768(&v).unwrap();
    /// ```
    #[cfg(feature = "botan3")]
    pub fn load_kyber768(key: &[u8]) -> Result<Privkey> {
        let obj = botan_init!(botan_privkey_load_kyber768, key.as_ptr())?;
        Ok(Privkey { obj })
    }

    /// Load a kyber 1024 private key
    ///
    /// # Examples
    ///
    /// ```
    /// let v = vec![0x42; 1568];
    /// let key = botan::Privkey::load_kyber1024(&v).unwrap();
    /// ```
    #[cfg(feature = "botan3")]
    pub fn load_kyber1024(key: &[u8]) -> Result<Privkey> {
        let obj = botan_init!(botan_privkey_load_kyber1024, key.as_ptr())?;
        Ok(Privkey { obj })
    }
    ...
    /// let key = botan::Privkey::load_kyber512(&v).unwrap();
    /// let v2 = key.get_kyber512_key().unwrap();
    /// assert_eq!(v, v2);
    /// ```
    pub fn get_kyber512_key(&self) -> Result<Vec<u8>> {
        let mut out = vec![0; 1632];
        botan_call!(botan_privkey_kyber512_get_privkey, self.obj, out.as_mut_ptr())?;
        Ok(out)
    }

    /// Get the Kyber768 private key
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// let key = botan::Privkey::load_kyber768(&v).unwrap();
    /// let v2 = key.get_kyber768_key().unwrap();
    /// assert_eq!(v, v2);
    /// ```
    pub fn get_kyber768_key(&self) -> Result<Vec<u8>> {
        let mut out = vec![0; 2400];
        botan_call!(botan_privkey_kyber768_get_privkey, self.obj, out.as_mut_ptr())?;
        Ok(out)
    }

    /// Get the Kyber1024 private key
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// let key = botan::Privkey::load_kyber1024(&v).unwrap();
    /// let v2 = key.get_kyber1024_key().unwrap();
    /// assert_eq!(v, v2);
    /// ```
    pub fn get_kyber1024_key(&self) -> Result<Vec<u8>> {
        let mut out = vec![0; 3168];
        botan_call!(botan_privkey_kyber1024_get_privkey, self.obj, out.as_mut_ptr())?;
        Ok(out)
    }
...
impl Pubkey {
    ...
    /// Load an Kyber512 key
    ///
    /// # Examples
    ///
    /// ```
    /// let v = vec![0x42; 1632];
    /// let key = botan::Pubkey::load_kyber512(&v).unwrap();
    /// ```
    pub fn load_kyber512(key: &[u8]) -> Result<Pubkey> {
        let obj = botan_init!(botan_pubkey_load_kyber512, key.as_ptr())?;
        Ok(Pubkey { obj })
    }

    /// Load an Kyber768 key
    ///
    /// # Examples
    ///
    /// ```
    /// let v = vec![0x42; 2400];
    /// let key = botan::Pubkey::load_kyber768(&v).unwrap();
    /// ```
    pub fn load_kyber768(key: &[u8]) -> Result<Pubkey> {
        let obj = botan_init!(botan_pubkey_load_kyber768, key.as_ptr())?;
        Ok(Pubkey { obj })
    }

    /// Load an Kyber1024 key
    ///
    /// # Examples
    ///
    /// ```
    /// let v = vec![0x42; 3168];
    /// let key = botan::Pubkey::load_kyber1024(&v).unwrap();
    /// ```
    pub fn load_kyber1024(key: &[u8]) -> Result<Pubkey> {
        let obj = botan_init!(botan_pubkey_load_kyber1024, key.as_ptr())?;
        Ok(Pubkey { obj })
    }
    ...
    pub fn get_kyber512_key(&self) -> Result<Vec<u8>> {
        ...
        botan_call!(botan_pubkey_kyber512_get_pubkey, self.obj, out.as_mut_ptr())?;
        Ok(out)
    }
    ...
    /// ```
    pub fn get_kyber768_key(&self) -> Result<Vec<u8>> {
        let mut out = vec![0; 1184];
        botan_call!(botan_pubkey_kyber768_get_pubkey, self.obj, out.as_mut_ptr())?;
        Ok(out)
    }

    /// Get an Kyber1024 key
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// ```
    pub fn get_kyber1024_key(&self) -> Result<Vec<u8>> {
        let mut out = vec![0; 1568];
        botan_call!(botan_pubkey_kyber1024_get_pubkey, self.obj, out.as_mut_ptr())?;
        Ok(out)
    }
